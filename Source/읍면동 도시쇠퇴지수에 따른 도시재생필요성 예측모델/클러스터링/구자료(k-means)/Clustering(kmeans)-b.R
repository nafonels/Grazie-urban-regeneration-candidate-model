...
pam.result<-pam(Nationwide[,-1],9) #클러스터링을 합니다 k-mediod로 했습니다.
...
#nc <- NbClust(teacher[,-1], min.nc=2, max.nc=15, method="median")
#par(mfrow=c(1,1))
...
pam.result<-pam(teacher[,-1],9) #k-medoid 클러스터링합니다.
#kmeans.result<-kmeans(teacher[,-1],9,nstart=100)
teacher$cluster<-as.factor(pam.result$cluster)
#write.csv(teacher,"report9_1.csv") #저장합니다
...
write.csv(student,"report9_2.csv") #저장합니다.